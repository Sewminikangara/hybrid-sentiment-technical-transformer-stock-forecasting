    ...
    def train(self, X_technical_train: np.ndarray, X_sentiment_train: np.ndarray,
        ...
        for epoch in range(epochs):
            ...
            if verbose and (epoch + 1) % 10 == 0:
                if X_technical_val is not None:
                    logger.info("Epoch [{epoch+1}/{epochs}] - Train Loss: {avg_train_loss:.6f}, Val Loss: {val_loss:.6f}")
                else:
                    logger.info("Epoch [{epoch+1}/{epochs}] - Train Loss: {avg_train_loss:.6f}")
...
if __name__ == "__main__":
    logger.info("Attention Fusion Hybrid Transformer")
    logger.info("Cross-attention mechanism for dynamic fusion")
    logger.info("\nModel Architecture:")
    logger.info("  Technical → Transformer → Encoded Technical")
    logger.info("  Sentiment → Transformer → Encoded Sentiment")
    logger.info("  Cross-Attention:")
    logger.info("    - Technical queries Sentiment (what sentiment is relevant?)")
    logger.info("    - Sentiment queries Technical (what technical patterns matter?)")
    logger.info("  → Fusion → Prediction")
