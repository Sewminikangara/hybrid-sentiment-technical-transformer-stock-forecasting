...
def main():
    ...
    view_mode = st.selectbox(
        "Select View:",
        ["Prediction", "Analysis", "Compare Models", "Portfolio Manager", "Batch Prediction", "Performance Dashboard", "Trading Bot", "Auto Trader", "About"],
        index=0
    )
    ...
    if view_mode == "Prediction":
        ...
        show_batch_prediction_tab(selected_model, is_forex)
...
def show_trading_bot_tab(stock, model_name, is_forex=False):
    ...
    This generates signals based on model predictions.
    
    To implement actual trading:
    1. Connect to broker API (Alpaca, Interactive Brokers)
                                    ...
                                    if signal != "HOLD":
                                        if st.button(f"⚡ Auto-Execute {signal} Trade"):
                                            ...
                                            if result['status'] == 'success':
                                                ...
                                                st.info(f"ℹ Trade skipped: {result['reason']}")
                                            else:
...
def show_portfolio_manager_tab(is_forex=False):
    """Portfolio Management - Enterprise Feature for Financial Institutions"""
    ...
    with col2:
        st.metric("📈 Total Portfolio Value", f"${total_allocation:,.2f}", help="Sum of all positions")
            ...
            with st.spinner("Running portfolio analysis..."):
                ...
                for item in portfolio_data:
                    ...
                    if len(pred_row) > 0 and pred_row.iloc[0]['status'] == 'success':
                        pred_data = {
                            'predicted_price': pred_row.iloc[0]['predicted_price'],
                            'confidence': pred_row.iloc[0]['confidence']
                        }
                    else:
                        pred_data = {
                ...
                with col1:
                    st.metric(
                        "Value at Risk (95%)",
                        f"${risk_report['var_95_dollars']:,.2f}",
                        f"-{risk_report['var_95_percent']:.2f}%",
                        delta_color="inverse",
                ...
                with col2:
                    st.metric(
                        "CVaR (95%)",
                        f"${risk_report['cvar_95_dollars']:,.2f}",
                        f"-{risk_report['cvar_95_percent']:.2f}%",
                        delta_color="inverse",
                ...
                with col3:
                    ...
                    st.metric(
                        "Sharpe Ratio",
                        f"{sharpe:.2f}",
                        "Excellent" if sharpe > 1.5 else ("Good" if sharpe > 1.0 else "Fair"),
                ...
                with col4:
                    ...
                    st.metric(
                        ...
                        "Strong" if diversification > 70 else ("Moderate" if diversification > 50 else "Weak"),
                ...
                with col2:
                    ...
                    ax.pie(allocation_df['Allocation (%)'], labels=allocation_df['Symbol'], 
                ...
                st.dataframe(
                    stress_df.style.format({
                        ...
                        'Loss ($)': '${:,.2f}',
                        'Loss (%)': '{:.1f}%'
                    }).background_gradient(subset=['Loss (%)'], cmap='RdYlGn_r', vmin=-30, vmax=30),
                    use_container_width=True
                )
...
def show_batch_prediction_tab(model_name='early_fusion', is_forex=False):
    """Batch Prediction - Enterprise Feature for Fintech Startups"""
    ...
    with col1:
        ...
        selected_assets = st.multiselect(
            "Select assets to predict",
            all_assets,
            default=default_assets,
            help="Select multiple assets for batch prediction"
        )
    ...
    with col3:
        min_confidence = st.slider("Min Confidence", 0.0, 1.0, 0.6, 0.05, help="Filter by minimum confidence")
    ...
    if st.button("🎯 Run Batch Prediction", type="primary"):
        if len(selected_assets) == 0:
            ...
            with st.spinner(f"Predicting {len(selected_assets)} assets in parallel..."):
                ...
                # Run batch prediction with risk metrics
                results_df = batch_predictor.predict_with_risk_metrics(
                    selected_assets, 
                ...
                # Filter by confidence (only if confidence column exists)
                ...
                if len(successful) > 0:
                    ...
                    available_cols = ['symbol', 'predicted_price', 'expected_return', 'confidence', 
                    ...
                    if 'Predicted Price' in display_df.columns:
                        format_dict['Predicted Price'] = '${:.2f}'
                    if 'Expected Return (%)' in display_df.columns:
                        format_dict['Expected Return (%)'] = '{:.2f}%'
                    if 'Confidence' in display_df.columns:
                        format_dict['Confidence'] = '{:.2f}'
                    if 'VaR 95%' in display_df.columns:
                        format_dict['VaR 95%'] = '{:.2f}%'
                    if 'CVaR 95%' in display_df.columns:
                        format_dict['CVaR 95%'] = '{:.2f}%'
                    if 'Sharpe Ratio' in display_df.columns:
                        format_dict['Sharpe Ratio'] = '{:.2f}'
                    
                    # Style the dataframe
                    styled_df = display_df.style.format(format_dict)
                    
                    # Add gradients if columns exist
                    if 'Expected Return (%)' in display_df.columns:
                    ...
                    opportunities = batch_predictor.rank_opportunities(
                        selected_assets, 
                        days=prediction_days, 
                        is_forex=is_forex,
                    ...
                    for _, row in display_df.iterrows():
                        ax2.annotate(
                            row['Symbol'],
                            (row['Confidence'], row['Expected Return (%)']),
                            xytext=(5, 5),
                    ...
                    st.warning("⚠️ No successful predictions met the confidence threshold")
...
# Get opportunities
opportunities = predictor.rank_opportunities(
    symbols={selected_assets},
    min_confidence={min_confidence}
)
...
def show_performance_dashboard_tab():
    """Performance Dashboard - Enterprise Feature for Investment Professionals"""
    ...
    with col1:
        st.metric(
            "Total Predictions",
            f"{dashboard_metrics['total_predictions']:,}",
    ...
    with col2:
        st.metric(
            "Overall MAPE",
            f"{dashboard_metrics['overall_mape']:.2f}%",
            f"Recent: {dashboard_metrics['recent_mape']:.2f}%",
    ...
    with col3:
        st.metric(
            "Direction Accuracy",
            f"{dashboard_metrics['overall_direction_accuracy']:.1f}%",
            f"Recent: {dashboard_metrics['recent_direction_accuracy']:.1f}%",
            help="Percentage of correct price direction predictions"
        )
    
    with col4:
        st.metric(
            "Best Model",
            dashboard_metrics['best_model'],
            help="Model with lowest error rate"
        )
    ...
    if len(calibration_df) > 0:
        ...
        with col2:
            # Line chart
            fig, ax = plt.subplots(figsize=(6, 4))
            ax.plot(calibration_df['Confidence Bucket'], calibration_df['Avg Error (%)'], 
    ...
    with col1:
        if st.button("📥 Export Performance Report"):
            ...
            st.download_button(
                ...
                data=report,
                file_name=f"performance_report_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.md",
                mime="text/markdown"
            )
